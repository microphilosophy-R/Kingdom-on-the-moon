...
const visitors: Visitor[] = [
  {
    ...
    offer: { give: { quantumCore: 3, knowledge: 6 }, take: { water: 6 }, tech: '日冕镜阵效率 +15%' },
  },
  {
    id: 'melu',
    name: '梅露·第九孢',
    ...
    offer: { give: { oxygen: 16, biomass: 12 }, take: { power: 10 }, tech: '生态圈年度水耗 -1' },
  },
  {
    id: 'orri',
    name: '欧里·无重力',
    ...
    offer: { give: { alloy: 22, quantumCore: 2 }, take: { regolith: 8, power: 12 }, tech: '星舰推进额外 +2%' },
  },
  {
    id: 'nix',
    name: '尼克斯·二十七',
    ...
    offer: { give: { knowledge: 14, currency: 8 }, take: { alloy: 10 }, tech: '王城政策维护费归零' },
  },
  {
    id: 'taro',
    name: '塔罗·掘井者',
    ...
    offer: { give: { regolith: 30, alloy: 9 }, take: { oxygen: 7 }, tech: '采掘署合金产出 +1' },
  },
  {
    id: 'evi',
    name: '伊芙·回声',
    ...
    offer: { give: { biomass: 10, luxury: 8 }, take: { power: 9 }, tech: '娱乐区研究 +1' },
  },
]
...
function App() {
  ...
  const yearlyNet = useMemo(() => projectAnnualNet({
    ...
    globalBonus: { ...buildResearchBonus(techs), ...(policy === 'ration' ? { biomass: 1 } : {}) },
...
function Palace({ policy, palaceLevel, techs, onPolicy, onSelectPalace }: { policy: 'ration' | 'mandate' | 'festival'; palaceLevel: number; techs: string[]; onPolicy: (policy: 'ration' | 'mandate' | 'festival') => void; onSelectPalace: () => void }) {
  ...
  return <div className="palace-layout"><section className="palace-hero"><div className="palace-mark"><Crown size={48} /></div><span className="eyebrow">钛金王城 · 皇家运行系统</span><h2>以算法签字，<br />以氧气执行。</h2><p>王城不是装饰。它把统治翻译为稳定的制度增益，并决定月民是否愿意留在这片没有故乡的土地。</p><button onClick={onSelectPalace}>审阅王城设施 <ChevronRight size={17} /></button></section><section className="policy-board"><div className="section-heading"><div><span className="eyebrow">可签发政令</span><h2>王城等级 {palaceLevel}/3</h2></div><p>同一时间只能执行一项政策。</p></div><div className="policy-cards">{policies.map(item => { const PolicyIcon = item.icon; const unlocked = palaceLevel >= item.level; return <button key={item.id} disabled={!unlocked} className={`${policy === item.id ? 'selected' : ''} ${!unlocked ? 'locked' : ''}`} onClick={() => onPolicy(item.id)}><span><PolicyIcon size={22} /></span><div><small>{unlocked ? `王城 ${item.level} 级已授权` : `王城 ${item.level} 级解锁`}</small><h3>{item.name}</h3><p>{item.detail}</p></div>{policy === item.id && <Check size={18} />}</button> })}</div></section><section className="tech-cabinet"><span className="eyebrow">档案柜 · 已获技术</span>{techs.length ? techs.map(tech => <p key={tech}><FlaskConical size={15} />{tech}</p>) : <p className="empty-tech">尚无外星技术入库。接待来自深空的异客以扩充王室档案。</p>}</section></div>
}
