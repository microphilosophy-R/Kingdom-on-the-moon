...
export function Shipyard({ facility, shipProgress, score, techs, productionMethods, facilityModifiers, onMethod, onBack }: ShipyardProps) {
        ...
        <section className="detail-command-column">
          <article className="construction-card expand">
            ...
            <div className="construction-resources">
              <span>完成度</span>
              <b className="research-stat-big ship-progress-number">{shipProgress}%</b>
              <span className="construction-days-pill">评分 {score}</span>
            </div>
            <div className="construction-resources" style={{ marginTop: '.32rem' }}>
              <span>材料总价值</span>
              <b className="research-stat-big">{Math.round(shipProjectTotalValue)}</b>
            </div>
            <div className="construction-resources">
              <span>在岗</span>
          ...
          <article className="construction-card shrink">
            <h3>星舰说明</h3>
            <p className="research-rule-text">千日试验以星舰完成度为核心结算，阶段投入会直接决定终局评分。御座号的每一寸龙鳞都是数据。</p>
          </article>
        </section>
      </div>
      ...
      <section className="special-content-block ship-meter">
        ...
        <div className="ship-stage-list">
          {shipProjectStages.map(stage => (
            <article key={stage.id} style={{ border: '1px solid var(--ui-line)', borderRadius: '5px', padding: '.52rem .62rem', background: 'var(--ui-surface)' }}>
